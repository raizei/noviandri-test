...
if ( ! class_exists( 'WP_Bootstrap_Navwalker' ) ) {
	...
	class WP_Bootstrap_Navwalker extends Walker_Nav_Menu {
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if($depth==0){
				$title = '<span class="pix-dropdown-title text-sm '.$add_classes.' pix-header-text">'.do_shortcode($title).'</span>';
			}else{
				$title = '<span class="pix-dropdown-title '.$add_classes.' text-body-default '.$this->dropTextClasses.'">'.do_shortcode($title).'</span>';
			}
