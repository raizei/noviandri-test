...
if(!$hide_top_area){
    ...
    }elseif( in_array(get_post_type(), $pagePostTypes) ){
        $type_prefix = 'pages';
    }
    ...
    $dividers_default_size = array(100, 250, 250, 150, 100, 200, 100,
        ...
        if($type_prefix == 'blog'){
            if( !empty(pix_get_option('pix-disable-blog-parallax')) ){
                if(pix_get_option('pix-disable-blog-parallax')){
                    $intro_parallax = '';
                }
            }
        }
            ...
            <div class="row d-flex h-100 justify-content-center">
                ...
                <div class="col-xs-12 col-lg-12">
                    <div class="<?php echo esc_attr( $intro_align ); ?> my-2">
                        ...
                        if($type_prefix == 'blog'){
                            if( !empty(pix_get_option('pix-disable-title-animation')) ){
                                if(pix_get_option('pix-disable-title-animation')){
                                    $title_sliding = '';
                                }
                            }
                        }
                            if( (!is_front_page() && !is_404() && !is_search() && !is_archive() && !is_author()) || $is_shop ){
                                if($is_shop_post){
                                    ...
                                    <?php
                                    // echo the_title( '<h1 class="'.$title_sliding.' h3 '.$text_class.' font-weight-bold" data-class="'.$text_class.'">', '</h1>', false );
                                }
                                ?>
                                <div>
                                <?php
                                    ...
                                    if($type_prefix == 'blog'){
                                        if( !empty(pix_get_option('pix-disable-blog-breadcrumbs')) ){
                                            if(pix_get_option('pix-disable-blog-breadcrumbs')){
