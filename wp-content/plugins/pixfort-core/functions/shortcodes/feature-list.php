	...
	function sc_pix_feature_list( $attr, $content = null ){
		...
		if(is_array($features_arr)){
			foreach ($features_arr as $key => $value) {
				...
				if(!empty($value['media_type'])) {
					if($value['media_type'] == "duo_icon"){
						if(!empty($value['pix_duo_icon'])) {
							$output .= '<div class="pix-mr-10 '.$i_color.'" style="width:1.5em;height:1.5em;position:relative;line-height:1em;text-align:center;">';
				...
				if(!empty($value['text'])) $output .= '<span class="'.$c_color.'">'. $value['text'] .'</span>';
				$output .= '</div>';
			}
		}
		$output .= '</div>';
