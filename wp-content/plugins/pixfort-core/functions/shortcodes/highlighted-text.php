...
if( ! function_exists( 'sc_pix_highlighted_text' ) )
{
	function sc_pix_highlighted_text( $attr, $content = null )
	{
        ...
        if($texts){
            ...
            $output .= '<'.$title_tag.' class="'.$class_names.'" data-anim-type="'. $animation .'" data-anim-delay="'.$delay.'">';
                ...
                if(!empty($value['is_highlighted'])){
                    if(!empty($value['highlight_color'])){
                        if($value['highlight_color']!='#ffd900'){
                            $customStyle = '.'.$el_id.'-'.$key.' { background-image: linear-gradient('.$value['highlight_color'].', '.$value['highlight_color'].') !important; }';
                            $classes .= ' '.$el_id.'-'.$key;
