...
if( ! function_exists( 'sc_pix_promo_box' ) ){
	...
	function sc_pix_promo_box( $attr, $content = null ){
		...
		$output .= '<div class="card w-100 h-100 bg-'.$overlay_color.'  pix-hover-item rounded-lg position-relative overflow-hidden text-white '.$class_names.'" '.$el_style.' '.$anim_type.' '.$anim_delay.' '.$jarallax.'>
		  <img src="'.$imgSrc.'" class="card-img pix-bg-image pix-img-scale h-100 '.$overlay_opacity.' '.$hover_overlay_opacity.'" alt="'.$title.'">
		  <a href="'.$link.'" '.$target_out.' class="card-img-overlay2 d-inline-block w-100 pix-img-overlay pix-p-20 d-flex align-items-end" '.$box_height.'>
		  	<div>';
			...
			$output .= '<span class="d-flex align-items-center '.$link_classes.'" '.$link_style.'><span>'.$link_text.'</span><i class="pixicon-angle-right pix-hover-right pix-hover-item pix-ml-10 font-weight-bold text-20"></i></span>
			</div>
		  </a>
		</div>';
